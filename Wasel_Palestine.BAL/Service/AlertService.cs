        ...
        public async Task<AlertResponse> CreateAlertAsync(AlertCreateRequest request, string userId, string ip, string userAgent)
        {
            ...
            try
            {
                ...
                foreach (var sub in subscribers)
                {
                    ...
                    {
                        string readUrl = $"http://localhost:32768/api/v1/Reports/mark-as-read?alertId={alert.Id}&userId={sub.UserId}";
                        string unsubscribeUrl = $"http://localhost:32768/api/v1/Alerts/unsubscribe?alertId={alert.Id}&userId={sub.UserId}";
                        string emailHtml = $@"
